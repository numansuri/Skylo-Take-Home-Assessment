...
This project was built with **Claude Code** (Anthropic's CLI-based AI coding assistant) and the **Superpowers plugin**, which provides structured development workflows on top of Claude Code. Skylo explicitly permits and encourages AI tool usage — transparency about tooling is a positive signal, not a caveat.

### Claude Code

[Claude Code](https://docs.anthropic.com/en/docs/claude-code) is Anthropic's agentic coding tool that operates directly in the terminal. It reads and edits files, runs commands, searches codebases, and manages git workflows — all within a conversational interface. Key features used in this project:

- **Multi-model routing** — Claude Code supports multiple Claude models within the same session. The git co-author tags on each commit show exactly which model was used for what:
  - **Claude Opus 4.6 (1M context)** — Used for architecture design, initial project scaffold, comprehensive README writing, and ML model justification. Co-authored commits: `7043617`, `a23dcd3`, `7724369`, `434ed3b`, `448b251`.
  - **Claude Sonnet 4.6** — Used for focused performance optimization and refactoring. Co-authored commits: `11a8ecf`, `e5e7a5d`, `decdbd2`, `fda7b9c`.
- **Tool-integrated workflow** — Rather than copy-pasting code between an editor and a chatbot, Claude Code reads files, runs `pytest`, executes `python main.py`, inspects outputs, and edits code directly — all within one session. This enabled tight iteration loops where test failures were caught and fixed immediately.
- **Git integration** — Claude Code staged changes, drafted commit messages, and created commits directly. Every commit message in this repo was generated by Claude Code based on the actual diff, then reviewed and approved before committing.

### Superpowers Plugin

The [Superpowers plugin](https://github.com/anthropics/claude-code-superpowers) adds structured development skills to Claude Code that enforce disciplined workflows — brainstorming before coding, planning before implementing, testing before claiming completion. Skills used in this project:

- **Brainstorming** (`superpowers:brainstorming`) — Before writing any code, used to explore the design space: why IsolationForest over One-Class SVM or autoencoders, where the LLM/deterministic boundary should sit, and how to structure the dual-detection approach (IF for multivariate outliers, EWMA for drift). This front-loaded the hard decisions so implementation could proceed without architectural second-guessing.

- **Writing Plans** (`superpowers:writing-plans`) — Used to create a step-by-step implementation plan that broke the project into phases with clear interfaces: telemetry simulator → detection agent → interpretation agent → API layer → dashboard → tests. Each phase had defined inputs and outputs, which is why the modules have clean boundaries (e.g., `DetectionResult` as the contract between detection and interpretation).

- **Test-Driven Development** (`superpowers:test-driven-development`) — The 16-test suite was developed alongside the implementation, not after. The `TestBatchProcessing` suite — which validates exact equivalence between batch and sequential processing — was written *before* the batch optimization, ensuring the optimization couldn't silently change detection behavior.

- **Systematic Debugging** (`superpowers:systematic-debugging`) — When the initial README claimed ~18 anomalies but the actual pipeline produced 67, this skill guided the diagnosis: the contamination parameter (0.05) only affects `predict()`, but our pipeline uses `score_samples()` with custom z-score thresholds, so the detection rate is governed by the threshold, not contamination. This led to the corrected documentation in commit `a23dcd3`.

- **Code Review** (`superpowers:requesting-code-review`) — After the performance optimization phase, code review identified that the detection module still imported and defined Agents SDK objects (`Agent`, `@function_tool` decorators, module-level agent definition) that were no longer called anywhere. This directly produced the cleanup in commit `fda7b9c`.

- **Subagent-Driven Development** (`superpowers:subagent-driven-development`) — Used during the performance optimization phase to dispatch parallel subagents for independent tasks. The vectorized batch detection (`e5e7a5d`) and parallel interpretation (`decdbd2`) optimizations were independent workstreams — subagents worked on them concurrently without blocking each other, then results were reviewed and committed sequentially.

- **Verification Before Completion** (`superpowers:verification-before-completion`) — Before marking any step complete, this skill required running verification commands (`pytest tests/ -v`, `python main.py`, checking the dashboard in-browser) and confirming actual output rather than assuming correctness.

### How the Tooling Shaped the Final Architecture

The structured workflow had a direct, traceable impact on the codebase:

1. **The LLM-free detection path** was not the original design. The initial commit (`7043617`) used the OpenAI Agents SDK for detection with `@function_tool`-decorated functions. During brainstorming and plan review, it became clear that the detection math (IsolationForest scoring, EWMA z-scores) is fully deterministic — wrapping it in an LLM agent added latency, cost, and non-determinism with no accuracy benefit. Commits `11a8ecf` through `fda7b9c` show the progressive removal of LLM from the detection path.

2. **Batch + parallel architecture** came from a planned optimization phase that identified two independent targets: vectorized detection (commit `e5e7a5d`) and concurrent interpretation (commit `decdbd2`). Because the plan identified these as independent, they could be implemented and tested separately.

3. **The README evolved with the code** — not written once at the end. Five of the ten commits touch the README because each architectural change triggered a documentation update. The ML justification section was rewritten in `a23dcd3` after systematic debugging revealed the initial metrics were wrong.

All code was reviewed, understood, and validated by the author. The architecture decisions were made during the brainstorming and planning phases, then implemented faithfully through the structured workflow.
